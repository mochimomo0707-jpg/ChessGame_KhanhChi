...
public class RankFrm extends javax.swing.JFrame {
    ...
    public void updateRankList(List<User> userList) {
        this.listUserStatics = userList;  // để mouseClick vẫn hoạt động
        DefaultTableModel model = (DefaultTableModel) rankTextArea.getModel();
        model.setRowCount(0);

        int stt = 1;
        for (User user : userList) {
            int games = user.getNumberOfGame();
            int wins = user.getNumberOfWin();
            int draws = user.getNumberOfDraw();
            int winRate = (games > 0) ? (wins * 100 / games) : 0;
