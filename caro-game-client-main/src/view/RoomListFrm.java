...
public class RoomListFrm extends javax.swing.JFrame {
    ...
    public RoomListFrm() {
        ...
        roomTextArea.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                int row = roomTextArea.rowAtPoint(evt.getPoint());
                int col = roomTextArea.columnAtPoint(evt.getPoint());

                // DEBUG
                System.out.println("Click: row=" + row + ", col=" + col);

                // Chỉ xử lý nếu click vào icon cột 1
                if (row >= 0 && col == 1) {
                    try {
                        isPlayThread = false;

                        String roomString = listRoom.get(row);
                        String roomIDStr = roomString.replaceAll("[^0-9]", "");
                        int roomID = Integer.parseInt(roomIDStr);

                        String password = listPassword.get(row);
                        if (!password.equals(" ")) {
                            password = JOptionPane.showInputDialog("Nhập mật khẩu phòng:");
                            if (password == null) {
                                return; // ấn Cancel
                            }
                        } else {
                        ...
                        System.out.println("Gửi lệnh: go-to-room," + roomID + "," + password);
                        Client.socketHandle.write("go-to-room," + roomID + "," + password);

                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(null, "Không thể vào phòng: " + ex.getMessage());
                        ex.printStackTrace();
                    }
                }
            }
    ...
    private void roomTextAreaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_roomTextAreaMouseClicked
        ...
        try {
            ...
            String roomString = listRoom.get(index);
            String roomIDStr = roomString.split(" ")[1].replace(":", "");
            int roomID = Integer.parseInt(roomIDStr);

            String password = listPassword.get(index);
            if (password.equals(" ")) {
                Client.socketHandle.write("join-room," + roomID);
// Tạm thời KHÔNG close view — chờ server phản hồi "go-to-room" rồi mới close

            } else {
                Client.closeView(Client.View.ROOM_LIST);
                Client.openView(Client.View.JOIN_ROOM_PASSWORD, roomID, password);
            }
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(rootPane, ex.getMessage());
        }
